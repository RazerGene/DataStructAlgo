package com.razergene.datastructalgo.greedy;
...
public class Min_priorityQueue
{
	...
	public Nodes Heap_Extract_Min()
	{
		...
		{
			a[i] = new Nodes(A[i].getCharacter(), A[i].getWeight(),
					A[i].getLchild(), A[i].getRchild());
		}
		A = a;
		Min_Heapify(1);
		return min;
	...
	/*
	 * public static void main(String[] args) { int[] a = { 15, 13, 9, 5, 12, 8,
	 * 7, 4, 0, 6, 2, 1 }; Min_priorityQueue mpq = new Min_priorityQueue(a);
	 * mpq.BuildMaxHeap(); mpq.Min_Heap_Insert(200); mpq.output();
	 * System.out.println("\n"+mpq.Heap_Extract_Min()); mpq.output();
	 * System.out.println("\n"+mpq.Heap_Minimum());
	 * 
	 * }
	 */
